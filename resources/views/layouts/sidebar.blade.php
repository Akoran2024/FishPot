<aside class="fixed inset-y-0 left-0 w-64 bg-primary-900 text-white transition-transform duration-300 transform z-30 overflow-y-auto shadow-2xl border-r border-primary-800">
    ...
    <nav class="mt-8 px-4 space-y-2">
        ...
        <a href="{{ route('admin.users.index') }}" class="flex items-center px-4 py-3 text-sm font-bold rounded-xl transition-colors {{ request()->routeIs('admin.users.*') ? 'bg-primary-800 text-white shadow-lg shadow-primary-900/50 border border-primary-700' : 'text-primary-100 hover:bg-primary-800 hover:text-white' }}">
            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
            Usuarios
        </a>

        <a href="{{ route('admin.products.index') }}" class="flex items-center px-4 py-3 text-sm font-bold rounded-xl transition-colors {{ request()->routeIs('admin.products.*') ? 'bg-primary-800 text-white shadow-lg shadow-primary-900/50 border border-primary-700' : 'text-primary-100 hover:bg-primary-800 hover:text-white' }}">
            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
            Productos
        </a>
        
        <a href="{{ route('admin.orders.index') }}" class="flex items-center px-4 py-3 text-sm font-bold rounded-xl transition-colors {{ request()->routeIs('admin.pedidos.*') ? 'bg-primary-800 text-white shadow-lg shadow-primary-900/50 border border-primary-700' : 'text-primary-100 hover:bg-primary-800 hover:text-white' }}">
